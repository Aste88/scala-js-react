...
object TodoApp extends TypedReactSpec with TypedEventListeners {
  ...
  def render(self: This) = {
    ...
    <section id="todoapp">
      <header id="header">
        <h1>todos</h1>
        <input id="new-todo" onChange={ self.onChange } onKeyUp={ self.onKeyPress } value={ self.state.text } placeholder="What needs to be done?" autofocus={ true }></input>
      </header>
      <section id="main">
        <input id="toggle-all" type="checkbox" checked={ activeTodoCount == 0 } onClick={ self.toggleAll } readOnly={ true }>
        </input>
        <ul id="todo-list">
          {
            for {
